...
class EmmaPolicy(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        if self.hparams.lr_scheduler == "linear_with_warmup":
            num_training_steps, num_warmup_steps = self.compute_warmup(
                self.num_training_steps(), self.hparams.num_warmup_steps
            )
    ...
    def training_step(self, batch, batch_idx) -> EmmaSeq2SeqLMOutput:
        """Training step."""
        output = self.emma(
    ...
    def validation_step(self, batch, batch_idx) -> EmmaSeq2SeqLMOutput:
        """Validation step."""
        output = self.emma(
            scene_features=batch.scene_features,
            scene_coordinates=batch.scene_coordinates,
